...
exports.findbyNome = (req, res) => {
    User.find(req.params.nome)
    .then(usuario => {
        if(!usuario) {
            return res.status(404).send({
                message: "Usuário desaparecido com este Nome: " + req.params.nome
            })          
        }
        res.send(usuario);
    }).catch(err => {
        if(err.kind === 'ObjectId') {
            return res.status(404).send({
                message: "Usuário não encontrado com o Nome: " + req.params.nome
            })               
        }
        return res.status(500).send({
            message: "Erro ao buscar usuário com o Nome: " + req.params.nome
        })
    })
}
